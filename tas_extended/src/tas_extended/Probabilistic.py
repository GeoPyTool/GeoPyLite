...
def Compare(df_train= pd.DataFrame(),df_test= pd.DataFrame(), x_label='x',y_label='x'):
    ...
    n = 1024
    x = np.linspace(35, 90, n)
    y = np.linspace(0, 20, n)
    data_whole = np.column_stack((x, y))
    ...
    for x, y in zip(x_test, y_test):
        ...
        test_probabilities /= np.max( max_prob_density)
...
plt.scatter(df_mugearite['SIO2_wt_calibred'], df_mugearite['ALL_Alkaline_wt_calibred'], color='blue', label='Training data',edgecolors= None,alpha=0.1)
